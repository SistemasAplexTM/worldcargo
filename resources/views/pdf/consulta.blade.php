        ...
        <tr>
            <td colspan="2" rowspan="5" style="width:300px;">
                @if (env('APP_DEPELOPER'))
                    <img class="img-circle" id="logo" height="120px" style="width: 100%"
                        src="{{ public_path() . '/storage/' }}/{{ (isset($agencia->logo) and $agencia->logo != '') ? trim($agencia->logo) : 'logo.png' }}"
                        style="width: 100%" />
                @else
                    <img alt="image" class="img-circle" id="logo" height="70px" style="width: 100%"
                        src="{{ asset('storage/') }}/{{ (isset($agencia->logo) and $agencia->logo != '') ? $agencia->logo : 'logo.png' }}"
                        style="width: 100%" />
                @endif
            </td>
            <td colspan="2" class="agency_title title_doc" style="">
                {{ (isset($agencia->descripcion) and $agencia->descripcion != '') ? $agencia->descripcion : '' }}</td>
        </tr>
        <tr>
            <td colspan="2" class="agency_title">
                {{ (isset($agencia->direccion) and $agencia->direccion != '') ? $agencia->direccion : '' }}</td>
        </tr>
        <tr>
            <td colspan="2" class="agency_title">
                {{ (isset($agencia->ciudad) and $agencia->ciudad != '') ? $agencia->ciudad : '' }},
                {{ (isset($agencia->agencia_depto_prefijo) and $agencia->agencia_depto_prefijo != '') ? $agencia->agencia_depto_prefijo : $agencia->depto }}
                {{ (isset($agencia->zip) and $agencia->zip != '') ? $agencia->zip : '' }}</td>
        </tr>
        <tr>
            <td colspan="2" class="agency_title">
                {{ (isset($agencia->telefono) and $agencia->telefono != '') ? $agencia->telefono : '' }}</td>
        </tr>
        <tr>
            <td colspan="2" class="agency_title">
                {{ (isset($agencia->email) and $agencia->email != '') ? $agencia->email : '' }}</td>
        </tr>
    </table>
