    ...
    <body>
      ...
      <?php //exit(); ?>
        @if(count($detalle) > 0)
            ...
            @endforeach
            @php
            $total_volumen_cft = ceil(number_format($total_volumen * 166 / 1728)); //cft = lxhxw / 1728 (pie cubico)
            $total_volumen_cmb = number_format(ceil(($total_volumen * 166 / 1728) / 35.315),0); //cmt = cft/35.315 (metro cubico)
             @endphp
        @endif
        <table>
          <tr>
            ...
            <tbody>
              @foreach($detalle as $val)
                <tr>
                  ...
                  <td class="detalle">{{ calculateFt($val->volumen) }} / {{ number_format(ceil(($val->volumen * 166 / 1728) / 35.315),0) }}</td>
                </tr>
              @endforeach
            </tbody>
          @else
