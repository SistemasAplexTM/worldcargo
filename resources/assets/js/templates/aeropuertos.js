$(document).ready(function () {
    llenarSelect('aeropuertos', 'localizacion', 'localizacion_id', 2);// module, tableName, id_campo
    $('#tbl-aeropuertos').DataTable({
        ajax: 'aeropuertos/all',
        columns: [
...
function edit(id,nombre,ciudad_id,ciudad,estado_id,estado,pais_id,pais){
    var data ={
        id:id,
        nombre: nombre,
        ciudad_id: ciudad_id,
...
var objVue = new Vue({
    ...
    methods:{
        ...
        rollBackDelete: function(data){
            var urlRestaurar = 'aeropuertos/restaurar/' + data.id;
        ...
        updateTable: function(){
            refreshTable('tbl-aeropuertos');
        },
        delete: function(data){
            this.formErrors = {};
            this.listErrors = {};
            if(data.logical === true){
                axios.get('aeropuertos/delete/' + data.id + '/' + data.logical).then(response => {
                    this.updateTable();
                    toastr.success("<div><p>Registro eliminado exitosamente.</p><button type='button' onclick='deshacerEliminar(" + data.id + ")' id='okBtn' class='btn btn-xs btn-danger pull-right'><i class='fa fa-reply'></i> Restaurar</button></div>");
                    toastr.options.closeButton = true;
                });
            }else{
                axios.delete('aeropuertos/' + data.id).then(response => {
                    this.updateTable();
